...
use boq_core::csi_masterformat::{MasterFormatMappingTable, apply_classification_overlay};
use boq_core::model::{
...
#[test]
fn masterformat_mapping_fixture_attaches_classification_without_support_promotion() {
    ...
    let annotations = first_item(&document)
        .try_multi_standard()
        .expect("MasterFormat annotations should decode");
    assert_eq!(annotations.classifications.len(), 1);
    let classification = &annotations.classifications[0];
    assert_eq!(classification.system, ClassificationSystem::CsiMasterFormat);
    assert_eq!(classification.code, "03 30 00");
    assert_eq!(
        classification.label.as_deref(),
        Some("Cast-in-Place Concrete")
    );
    assert_eq!(
        classification
            .source
        ...
        Some("tests/fixtures/synthetic/masterformat_mapping.json")
    );
}

#[test]
fn obra_adapter_exports_masterformat_when_adapter_is_already_supported() {
    let mut document = supported_ava_document();
    ...
    assert!(import.classifications.iter().any(|classification| {
        classification.system_code == "csi_masterformat"
            && classification.external_code == "03 30 00"
            && classification.external_title.as_deref() == Some("Cast-in-Place Concrete")
    }));
}

#[test]
fn parse_only_document_remains_not_adapter_supported_after_masterformat_overlay() {
    ...
    let error = ObraImportDocument::try_from_gaeb(&document)
        .expect_err("MasterFormat overlay must not promote parse-only adapter support");
    assert_eq!(error.code, "obra_adapter_not_supported");
    assert_eq!(document.support_status, SupportStatus::SupportedParseOnly);
}

#[test]
fn invalid_masterformat_fixture_variants_are_reported_explicitly() {
    let cases = [
        ("not json", "masterformat_mapping_invalid_json"),
        (
            r#"{"source_uri":"fixture","rules":[]}"#,
            "masterformat_mapping_empty",
        ),
        (
            r#"{"source_uri":"fixture","rules":[{"match_text":" ","code":"03 30 00","label":"Concrete"}]}"#,
            "masterformat_mapping_empty_match",
        ),
        (
            r#"{"source_uri":"fixture","rules":[{"match_text":"Concrete","code":"033000","label":"Concrete"}]}"#,
            "masterformat_mapping_invalid_code",
        ),
        (
            r#"{"source_uri":"fixture","rules":[{"match_text":"Concrete","code":"03 30 00","label":" "}]}"#,
    ...
    for (input, expected_code) in cases {
        let finding =
            MasterFormatMappingTable::from_json_str(input).expect_err("fixture should be rejected");
...
fn overlay_matches_nested_rich_text_without_duplicate_classifications() {
    ...
    assert_eq!(
        annotations
            .classifications
            .iter()
            .filter(|classification| classification.system == ClassificationSystem::CsiMasterFormat)
            .count(),
        1
    );
...
fn overlay_matches_xhtml_long_text_case_insensitively() {
    ...
    let annotations = nested_item(&document)
        .try_multi_standard()
        .expect("annotation metadata should decode");
    assert_eq!(annotations.classifications[0].code, "03 30 00");
}

fn mapping_table() -> MasterFormatMappingTable {
    MasterFormatMappingTable::from_json_str(include_str!(
        "fixtures/synthetic/masterformat_mapping.json"
    ))
    .expect("synthetic MasterFormat fixture should load")
}
...
fn document_with_item(item: BoqItem) -> GaebDocument {
    GaebDocument {
        source: SourceProvenance {
            source_uri: Some("synthetic/masterformat-rich-text.x81".to_owned()),
        ...
        summary: GaebDocumentSummary {
            ...
            phase: None,
            title: Some("Synthetic MasterFormat".to_owned()),
            project_name: None,
        },
        boq: Boq {
            title: "Synthetic MasterFormat".to_owned(),
            nodes: vec![BoqNode {
                ordinal: "1".to_owned(),
