...
use boq_core::nlsfb::{NlSfbMappingTable, apply_classification_overlay};
use boq_core::support::{SupportCapabilities, SupportStatus};
use rust_decimal::Decimal;
use serde_json::json;

#[test]
fn nlsfb_mapping_fixture_attaches_classification_without_support_promotion() {
    ...
    let annotations = first_item(&document)
        .try_multi_standard()
        .expect("NL-SfB annotations should decode");
    assert_eq!(annotations.classifications.len(), 1);
    let classification = &annotations.classifications[0];
    assert_eq!(classification.system, ClassificationSystem::NlSfb);
    assert_eq!(classification.code, "21.11");
    assert_eq!(
        classification.label.as_deref(),
        Some("Synthetic concrete construction")
    );
    assert_eq!(
        classification
            .source
        ...
        Some("tests/fixtures/synthetic/nlsfb_mapping.json")
    );
}

#[test]
fn obra_adapter_exports_nlsfb_when_adapter_is_already_supported() {
    let mut document = supported_ava_document();
    ...
    assert!(import.classifications.iter().any(|classification| {
        classification.system_code == "nlsfb"
            && classification.external_code == "21.11"
            && classification.external_title.as_deref() == Some("Synthetic concrete construction")
    }));
}

#[test]
fn parse_only_document_remains_not_adapter_supported_after_nlsfb_overlay() {
    ...
    let error = ObraImportDocument::try_from_gaeb(&document)
        .expect_err("NL-SfB overlay must not promote parse-only adapter support");
    assert_eq!(error.code, "obra_adapter_not_supported");
    assert_eq!(document.support_status, SupportStatus::SupportedParseOnly);
}

#[test]
fn invalid_nlsfb_fixture_variants_are_reported_explicitly() {
    let cases = [
        ("not json", "nlsfb_mapping_invalid_json"),
        (
            r#"{"source_uri":"fixture","rules":[]}"#,
            "nlsfb_mapping_empty",
        ),
        (
            r#"{"source_uri":"fixture","rules":[{"match_text":" ","code":"21.11","label":"Wall"}]}"#,
            "nlsfb_mapping_empty_match",
        ),
        (
            r#"{"source_uri":"fixture","rules":[{"match_text":"Wall","code":"2111","label":"Wall"}]}"#,
            "nlsfb_mapping_invalid_code",
        ),
        (
            r#"{"source_uri":"fixture","rules":[{"match_text":"Wall","code":"21.11","label":" "}]}"#,
    ...
    for (input, expected_code) in cases {
        let finding =
            NlSfbMappingTable::from_json_str(input).expect_err("fixture should be rejected");
...
fn overlay_matches_nested_rich_text_without_duplicate_classifications() {
    ...
    assert_eq!(
        annotations
            .classifications
            .iter()
            .filter(|classification| classification.system == ClassificationSystem::NlSfb)
            .count(),
        1
    );
...
fn overlay_matches_xhtml_long_text_case_insensitively() {
    ...
    let annotations = nested_item(&document)
        .try_multi_standard()
        .expect("annotation metadata should decode");
    assert_eq!(annotations.classifications[0].code, "21.11");
}

fn mapping_table() -> NlSfbMappingTable {
    NlSfbMappingTable::from_json_str(include_str!("fixtures/synthetic/nlsfb_mapping.json"))
        .expect("synthetic NL-SfB fixture should load")
}
...
fn document_with_item(item: BoqItem) -> GaebDocument {
    GaebDocument {
        source: SourceProvenance {
            source_uri: Some("synthetic/nlsfb-rich-text.x81".to_owned()),
        ...
        summary: GaebDocumentSummary {
            ...
            phase: None,
            title: Some("Synthetic NL-SfB".to_owned()),
            project_name: None,
        },
        boq: Boq {
            title: "Synthetic NL-SfB".to_owned(),
            nodes: vec![BoqNode {
                ordinal: "1".to_owned(),
