...
| GAEB DA XML X89 Rechnung | Synthetic invoice-domain parser and Obra billing draft boundary | parser/billing-draft evidence; no manifest support promotion | Parse invoice-domain data, derive `ObraBillingDraft`, and inspect readiness/loss findings; no XRechnung generation, production billing readiness, export, roundtrip, or certification is implied. |
