...
The `boq_core::catalogo` module can apply a deterministic, fixture-backed Catálogo de Conceptos and Cuadro de Precios overlay to parsed BoQ items. It writes synthetic `CatalogoConceptos` and `CuadroPrecios` `PriceCatalogReference` values into `MultiStandardAnnotations`, preserves the mapping source as provenance, and records price-table factors or other non-applied assumptions as explicit loss findings.
